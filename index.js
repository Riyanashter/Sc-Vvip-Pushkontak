...
async function startSesi() {
    ...
    x.ev.on("connection.update", ({ connection }) => {
      if (connection === "open") {
        console.log("KONEKSI BNG YAN " + "Terhubung (" + x.user?.["id"]["split"](":")[0] + ")")
      }
      if (connection === "close") {
        startSesi()
      }
      if (connection === "connecting") {
        if (x.user) {
          console.log("SC PAIRING BY BNG YAN\nYT : @BANGYANOFFICIAL\nCREATOR : BNGYAN\nSELAMAT MENIKMATI")
        } else if (!useQR && !useCODE) {
          console.log("CONNECTION BNG YAN " + "Autentikasi Dibutuhkan\nGunakan Perintah \x1B[36mnpm start\x1B[0m untuk terhubung menggunakan nomor telepon\n\n\x1B[1m\x1B[41m Full Tutorial Check di Youtube: @BANGYANOFFICIAL \x1B[0m\n\n")
        }
      }
    })
